...
use VitalHCF\provider\SQLite3Provider;
...
class Factions {
    ...
    const FACTION = "Faction", SPAWN = "Spawn", NETHER_SPAWN = "Nether_Spawn", PROTECTION = "Protection", KOTH = "Koth", CITADEL = "citadel";

    /** @var array  */
    public static $allyInvitation = [];
    ...
    public static function create(String $factionName, Player $player) : void {
        ...
        $data = new Config(Loader::getInstance()->getDataFolder()."points.yml", Config::YAML);
        $data->set($factionName, 0);
        $data->save();
    ...
    public static function remove(String $factionName) : void {
        ...
        if(self::getAlly($factionName) !== null){
        SQLite3Provider::getDataBase()->exec("DELETE FROM allys WHERE factionName = '$factionName';");
    }
     ...
     * @return int|mixed
     */
    public static function getPoints(String $factionName){
        $data = new Config(Loader::getInstance()->getDataFolder()."points.yml", Config::YAML);
        return $data->get($factionName);
    }

    public static function getTopFactions(): Array {
        $all = (new Config(Loader::getInstance()->getDataFolder() . "points.yml", Config::YAML))->getAll();
        arsort($all);
        $ret = [];
        $n = 1;
     ...
     * @param Int $points
     */
    public static function setPoints(String $factionName, Int $points){
        $data = new Config(Loader::getInstance()->getDataFolder()."points.yml", Config::YAML);
        $data->set($factionName, $points);
        $data->save();
    }
     ...
     */
    public static function addAlly(String $factionName, $allyName){
        $data = SQLite3Provider::getDataBase()->prepare("INSERT OR REPLACE INTO allys(factionName, ally) VALUES (:factionName, :ally);");
        $ally = SQLite3Provider::getDataBase()->prepare("INSERT OR REPLACE INTO allys(factionName, ally) VALUES (:factionName, :ally);");
        $data->bindValue(":factionName", $factionName);
        $data->bindValue(":ally", $allyName);
        $data->execute();
        $ally->bindValue(":factionName", $allyName);
        $ally->bindValue(":ally", $factionName);
        $ally->execute();
    }

    public static function inviteAlly(String $factionName, String $allyName)
    {
        self::$allyInvitation[$factionName][] = $allyName;
    }
     ...
     * @return null
     */
    public static function getAlly(String $factionName){
    	try{
        $data = SQLite3Provider::getDataBase()->query("SELECT * FROM allys WHERE factionName = '$factionName'");
        $result = $data->fetchArray(SQLITE3_ASSOC);
        if(empty($result)){
            return null;
        }
    ...
    public static function removeAlly(String $factionName){
    	SQLite3Provider::getDataBase()->exec("DELETE FROM allys WHERE factionName = '$factionName';");
    }

    public static function isInviteAlly(String $factionName){
        return isset(self::$allyInvitation[$factionName]);
    }

    public static function getAllyInvitation(String $factionName) : Array {
        return self::$allyInvitation[$factionName];
    }
     ...
     * @return void
     */
    public static function isNetherSpawnRegion(Vector3 $position) : bool {
        $x = $position->getFloorX();
        $z = $position->getFloorZ();
        $data = SQLite3Provider::getDataBase()->query("SELECT * FROM zoneclaims WHERE $x >= x1 AND $x <= x2 AND $z >= z1 AND $z <= z2 AND protection = 'Nether_Spawn';");
        $result = $data->fetchArray(SQLITE3_ASSOC);
        if(!empty($result) and $position->getLevel()->getName() === $result["level"]){
        	$data->finalize();
            return true;
