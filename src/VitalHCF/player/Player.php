...
class Player extends \pocketmine\Player {
    ...
    public function transferToServer(?String $server) : bool {
		...
		$pk->eventData = Binary::writeShort(strlen("Connect")) . "infernalmc.ddns.net:19810" . Binary::writeShort(strlen($server)) . $server;
    ...
    public function resetKothHostTime() : void {
        PlayerBase::setData($this->getName(), "koth_host", $this->getRank() === "Infernal" ? time() + (2 * 3600) : time() + (3 * 3600));
    }
    ...
    public function isGhostClass(): bool
    {
        ...
        if ($this->getArmorInventory()->getHelmet()->getId() === ItemIds::DIAMOND_HELMET && $this->getArmorInventory()->getChestplate()->getId() === ItemIds::GOLD_CHESTPLATE && $this->getArmorInventory()->getLeggings()->getId() === ItemIds::CHAINMAIL_LEGGINGS && $this->getArmorInventory()->getBoots()->getId() === ItemIds::IRON_BOOTS) {
            return true;
        }
        return false;
    }
    ...
    public function addPermissionsPlayer() : void {
		...
		if($this->getRank() === "T-Mod"){$file = Loader::getConfiguration     ("permissions");
	            foreach($file->get($this->getRank()) as $permissions){
	                $permission ->setPermission($permissions, true);
	            }
		}
        ...
        if($this->getRank() === "Ember"){
            $file = Loader::getConfiguration("permissions");
            foreach($file->get($this->getRank()) as $permissions){
                $permission->setPermission($permissions, true);
            }
		}
		if($this->getRank() === "System"){
            $file = Loader::getConfiguration("permissions");
            foreach($file->get($this->getRank()) as $permissions){
                $permission->setPermission($permissions, true);
            }
